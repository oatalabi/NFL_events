...
def handle_error(error_code):
    try:
        error_code = int(str(error_code)) #handle any error exception in external API
    except:
        error_code = 500 #handle any internal server error
...
def get_scoreboard_data(base_url, start_date, end_date):
    scoreboard_api_url = f'{base_url}/scoreboard/NFL/{start_date}/{end_date}.json?api_key={API_KEY}'  

    # handle error from external API call
    try:
        scoreboard_response = requests.get(scoreboard_api_url, headers={"Accept": "application/json"})
        scoreboard_response.raise_for_status()
    except requests.exceptions.RequestException as err:
        raise Exception(scoreboard_response.status_code)

    scoreboard_data = scoreboard_response.json()['results']
    return scoreboard_data

def get_team_rankings(base_url):
    team_rankings_api_url = f'{base_url}/team_rankings/NFL.json?api_key={API_KEY}'    

    # handle error from external API call
    try:
        team_rankings_response = requests.get(team_rankings_api_url, headers={"Accept": "application/json"})
        team_rankings_response.raise_for_status()
    except requests.exceptions.RequestException as err:
        raise Exception(team_rankings_response.status_code)

    team_rankings_data = team_rankings_response.json()['results']['data']
...
def getEvents():
    ...
    scoreboard_results = get_scoreboard_data(base_url, start_date, end_date)
    rankings_results = get_team_rankings(base_url)
